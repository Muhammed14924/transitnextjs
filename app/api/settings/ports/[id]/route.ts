...
export async function PUT(
  ...
  try {
    ...
    const updated = await prisma.ports.update({
      where: { id },
      data: body,
    });
    ...
    console.error("PUT Ports API error:", error);
    return serverError("Failed to update port");
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    console.error("DELETE Ports API error:", error);
    return serverError("Failed to delete port");
  }
}
